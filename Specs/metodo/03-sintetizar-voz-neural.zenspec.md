...
texto curto, chama a API com a **voz masculina `pt-BR-Chirp3-HD-Orus`**
(natural, próxima, firme e grave), devolve o MP3 em base64 e guarda em
cache — o front toca e só cai para a voz local do navegador se este
programa falhar.
...
- Voz: `TTS_VOICE` se configurado, senão `pt-BR-Chirp3-HD-Orus` (**masculina natural, próxima, firme e grave** — combina com a marca).
- Tom: `TTS_RATE` (default **1.0**). Vozes **Chirp3 HD não aceitam `pitch`** → o backend força `pitch: 0` para elas (o grave vem da própria voz); nas demais, usa `TTS_PITCH` (default **-2**). Encoding MP3.
