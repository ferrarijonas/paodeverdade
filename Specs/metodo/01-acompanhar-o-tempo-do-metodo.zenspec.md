...
- A fala usa a **voz masculina neural do Google** (`?acao=tts`, `pt-BR-Chirp3-HD-Orus`) quando `dadosMetodo.tts` é verdadeiro; se a síntese falhar, cai para a **melhor voz pt-BR local** (preferência por `Online`/`Natural`). Texto **sóbrio e direto** (`Terceira dobra.`, `Hora de modelar a massa.`, `Hora de ir para o frio.`).
