...
- A fala usa a **voz neural do Google** (`?acao=tts`, voz `pt-BR-Neural2-C`) quando `dadosMetodo.tts` é verdadeiro; se a síntese falhar, cai para a **melhor voz pt-BR local** (preferência por `Online`/`Natural`). Texto sempre **natural por extenso** (`Terceira dobra`, `Hora de modelar a massa`, `Hora de ir para o frio`).
